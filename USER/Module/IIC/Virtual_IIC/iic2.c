...
#define SDA_READ   ((I2C_MONI_SDA_GPIO2->IDR & I2C_MONI_SDA_PIN2)?1:0)
...
// encoder：编码器结构体指针
void MT6701_SetZero_2(MT6701_Encoder_t *encoder) {
    ...
    encoder->raw_zero =get_raw_angle_2();
}
 void MT6701_Update_2(MT6701_Encoder_t *encoder) {
    // 步骤1：仅读取1次原始角度（避免I2C冗余读取）
    encoder->raw_angle = get_raw_angle_2();

    // 步骤2：计算单圈相对零点的实际角度
    get_real_angle_2(encoder);
